...
Rust-powered Model Context Protocol server for [PanDB](https://github.com/t8y2/dbx). It lets MCP-compatible AI agents inspect schemas and run safe database operations using connections configured in PanDB.
...
Set `DBX_WEB_URL` to use a deployed PanDB Web backend instead of local desktop storage:
...
`DBX_WEB_PASSWORD` is the password used on the PanDB Web login page. Desktop-local mode does not use it. Desktop UI tools are hidden in Web mode.
...
Choose **Read only**, **Data read/write**, or **Full access** in PanDB instead of placing permission flags in client configuration. Updated servers do not let `DBX_MCP_ALLOW_WRITES` or `DBX_MCP_ALLOW_DANGEROUS_SQL` widen the PanDB policy. For upgrade compatibility, `DBX_MCP_ALLOW_WRITES=0` (or `false`) keeps MCP read-only until a central policy is saved for the first time; the legacy permission variables are ignored afterward.
...
| `DBX_WEB_PASSWORD` | Authenticate to the PanDB Web backend |
...
PanDB MCP Server 是 [PanDB](https://github.com/t8y2/dbx) 的 Rust MCP 服务，让 Claude Code、Cursor、Windsurf 等兼容 MCP 的 AI 工具使用 PanDB 中已有的连接查询数据库。
...
通过 `DBX_DATA_DIR` 覆盖默认目录。Windows 便携版应指向 `PanDB.exe` 同级、包含 `dbx.db` 的 `data` 文件夹。
...
请在 PanDB 中选择 **只读**、**数据读写** 或 **完全访问**，不要在客户端配置中放置权限开关。新版 Server 不允许 `DBX_MCP_ALLOW_WRITES` 或 `DBX_MCP_ALLOW_DANGEROUS_SQL` 放宽 PanDB 中央策略。为兼容升级，在中央策略首次保存前，`DBX_MCP_ALLOW_WRITES=0`（或 `false`）仍会保持 MCP 只读；策略保存后旧权限变量即被忽略。
...
| `DBX_WEB_PASSWORD` | PanDB Web 登录密码 |
