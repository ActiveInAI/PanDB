...
fn show_recovery_failure_message() {
    ...
    let locale = sys_locale::get_locale().unwrap_or_default().to_ascii_lowercase();
    let body = if locale.starts_with("zh") {
        format!("PanDB 已尝试企业环境兼容模式，但主窗口仍未创建。\n\n请将此日志发给维护者：{log_path}")
    } else {
        format!(
            "PanDB tried enterprise environment compatibility mode, but the main window was still not created.\n\nPlease send this log to the maintainer: {log_path}"
        )
    };
    windows_ok_message("PanDB", &body);
}
