...
export default withEnglishFallback({
  ...
  updates: {
    ...
    portableAutomaticUpdate: "PanDB 将下载已签名的便携版 ZIP，退出后仅替换 PanDB.exe 并自动重启。portable.dbx 和 data 会保留。",
  ...
  sqlLibrary: {
    title: "SQL 库",
    storageHelp: "SQL 库内容保存在 PanDB 应用数据中，并非磁盘上的独立 .sql 文件。\n如需在文件系统中使用，请导出 SQL 库。",
    unassociated: "未关联",
  ...
  connection: {
    ...
    rememberPasswordSaveFailed: "连接成功，但 PanDB 无法记住密码：{message}",
  ...
  grid: {
    ...
    conditionalBulkEditConfirmMessage: "该操作会立即更新执行时条件匹配的全部行，包括尚未加载的行；PanDB 无法为这些行生成可靠的回滚 SQL。",
  ...
  backendErrors: {
    ...
    transaction: {
      sessionExpired: "手动事务因 {timeoutSecs} 秒无操作已过期并回滚，PanDB 将自动开始新事务。",
    },
    legacy: "后端返回了错误。",
    unknown: "发生了未预期的后端错误。",
  },
  ...
  ai: {
    ...
    closeWithRunsDescription: "退出 PanDB 会中断 {count} 个正在运行或排队的 AI 任务。你也可以取消退出并等待任务完成。",
    ...
    productionWriteBlocked: "AI 不能在生产数据库执行数据或结构变更。请核对此 SQL 后在 PanDB 中手动执行。",
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "配置覆盖了由 PanDB 管理的 MCP 环境变量。请从供应商配置中移除 PanDB_MCP_* 变量。",
      ...
      piAgentEnvReserved: "配置覆盖了由 PanDB 管理的 Pi 或 MCP 环境变量。请移除 PanDB_PI_* 和 PanDB_MCP_* 变量。",
      ...
      openCodeEnvReserved: "配置覆盖了由 OpenCode 或 PanDB 管理的环境变量。请移除 OPENCODE 控制变量和 PanDB_MCP_* 变量。",
      openCodeNotAuthenticated: "OpenCode 尚未配置已认证的模型提供商。请执行 `opencode auth login` 或在 OpenCode 中配置提供商后重试。",
      openCodeMcpStartupFailed: "OpenCode 无法启动受限范围的 PanDB MCP Server。请检查 设置 > MCP 和下方诊断详情。",
      ...
      grokCliEnvReserved: "覆盖了 PanDB 管理的 MCP 环境变量。请从提供商配置中移除 PanDB_MCP_* 变量。",
      grokCliNotAuthenticated: "Grok CLI 尚未登录。请在终端执行 `grok login` 后重试。",
      grokCliMcpStartupFailed: "Grok 已加载 MCP 配置，但无法启动 PanDB MCP Server。请检查 设置 > MCP 与下方诊断信息。",
      grokCliCommandLineTooLong: "命令行过长，Windows 拒绝执行 Grok CLI。请更新 PanDB 并使用生成的 prompt 文件重试。",
      ...
      codeBuddyEnvReserved: "覆盖了 PanDB 管理的 MCP 环境变量。请从提供商配置中移除 PanDB_MCP_* 变量。",
      codeBuddyNotAuthenticated: "CodeBuddy Code 尚未登录。请在终端打开 `codebuddy`，完成登录后重试。",
      codeBuddyTimeout: "CodeBuddy Code CLI 未在操作超时前响应。",
      codeBuddyMcpConfigInvalid: "CodeBuddy Code 无法读取临时 PanDB MCP 配置。请检查下方诊断信息。",
      codeBuddyMcpStartupFailed: "CodeBuddy Code 已加载 MCP 配置，但无法启动 PanDB MCP Server。请检查 设置 > MCP 与下方诊断信息。",
      ...
      qoderEnvReserved: "配置覆盖了由 PanDB 管理的 MCP 环境变量。请从供应商配置中移除 PanDB_MCP_* 变量。",
      ...
      qoderMcpConfigInvalid: "Qoder CLI 无法读取临时 PanDB MCP 配置。请检查 Qoder CLI 版本和下方诊断信息。",
      qoderMcpStartupFailed: "Qoder CLI 已加载 MCP 配置，但无法启动 PanDB MCP Server。请检查 设置 > MCP 和下方诊断信息。",
      ...
      cursorEnvReserved: "配置覆盖了由 Cursor 或 PanDB 管理的环境变量。请移除 CURSOR_CONFIG_DIR、CURSOR_DATA_DIR 和 PanDB_MCP_* 变量。",
      cursorNotAuthenticated: "Cursor CLI 尚未登录。请在终端执行 `agent login` 后重试。",
      cursorMcpStartupFailed: "Cursor 无法启动受限范围的 PanDB MCP Server。请检查 设置 > MCP 和下方诊断详情。",
  ...
  sqlServerTrace: {
    ...
    cleanedStaleSessions: "已清理 {count} 个超时遗留的 PanDB 跟踪会话。",
  ...
  settings: {
    ...
    syncWebDavWebDescription: "Web 模式下由 PanDB 服务端访问 WebDAV，不受浏览器 CORS 限制；保存的密码会留在当前 PanDB 实例中。",
    ...
    syncRememberWebDavPasswordDescription: "密码会加密保存在当前 PanDB 实例中，不会同步到 WebDAV，也不会替代同步密码。",
    ...
    syncSecretsPassphraseDescription: "这个密码只用于加密和恢复敏感信息，并会加密保存在当前 PanDB 实例中；在其他实例恢复时需要再次输入。",
    syncTest: "测试",
    syncUpload: "上传",
    syncDownload: "下载",
    ...
    appCloseUnsavedTabsModeDescription: "选择退出 PanDB 时如何处理有未保存内容的 SQL 标签页。",
    ...
    appCloseDraftPersistFailed: "PanDB 无法保存打开的标签页草稿，因此已取消退出。请检查磁盘空间和文件权限后重试。详情：{message}",
    ...
    mcpUninstalling: "卸载中...",
    mcpUninstallConfirm: "确定要卸载 PanDB MCP Server 吗？卸载后，已配置的 MCP 客户端将无法启动它。",
    ...
    mcpNpmBoundary: "PanDB 负责检查 MCP 状态；安装、升级和卸载由检测到的 npm 或 pnpm 运行环境执行。",
    mcpRefresh: "重新检查",
    mcpGuide: "MCP 指南",
  ...
  driverStore: {
    ...
    offlineExportSecurityHint: "仅包含 PanDB 托管的驱动产物及所需 JRE；本地路径、配置和外部驱动不会被导出。",
    ...
    offlineExportUnavailable_unmanagedInstall: "该产物未被记录为 PanDB 托管安装。",
    ...
    offlineExportUnavailable_missingManagedJre: "尚未安装所需的 PanDB 托管 JRE。",
    offlineExportUnavailable_invalidManagedJre: "所需的 PanDB 托管 JRE 无效或不安全。",
    refresh: "刷新",
    ...
    update: "更新",
    localInstall: "本地安装",
    localInstallHint: "适用于完全内网环境：在另一台可联网的电脑上，从 PanDB 的 GitHub Releases 页面下载官方 dbx-jdbc-plugin-*.zip 安装包，拷贝到本机后在此选择该文件——本机无需联网即可完成安装。",
    runtimeDrivers: "运行时",
    storageTab: "存储",
    runtimeTitle: "驱动运行时",
  ...
  nacos: {
    ...
    protectedRole: "PanDB 不允许移除内置管理员角色。",
    close: "关闭",
    ...
    namespaceId: "命名空间 ID",
    namespaceIdPlaceholder: "留空则由 PanDB 自动生成 UUID",
    namespaceName: "命名空间名",
    ...
    nacosDetectAccessibleNamespacesHint: "PanDB 会校验「{name}」中每个命名空间的配置与服务读取权限后再供选择。",
    ...
    nacosV3ConsoleManagedNamespacesHint: "当前使用 Nacos 3 Console API（通常为 8080）。可填写命名空间名称或 ID；PanDB 能读取目录时会自动将名称转换为实际 ID。",
    rnacosNamespaceAccessScopeHint: "配置了 r-nacos Console 用户资料后，PanDB 会根据当前用户的命名空间权限确定访问范围。",
    nacosAuth: "认证",
