...
export default withEnglishFallback({
  ...
  updates: {
    ...
    portableAutomaticUpdate: "PanDB 將下載已簽署的可攜版 ZIP，結束後僅替換 PanDB.exe 並自動重新啟動。portable.dbx 和 data 會保留。",
  ...
  sqlLibrary: {
    title: "SQL 庫",
    storageHelp: "SQL 庫內容儲存在 PanDB 應用程式資料中，並非磁碟上的獨立 .sql 檔案。\n如需在檔案系統中使用，請匯出 SQL 庫。",
    unassociated: "未關聯",
  ...
  connection: {
    ...
    rememberPasswordSaveFailed: "連線成功，但 PanDB 無法記住密碼：{message}",
  ...
  grid: {
    ...
    conditionalBulkEditConfirmMessage: "此操作會立即更新執行時符合條件的全部資料列，包括尚未載入的資料列；PanDB 無法為這些資料列產生可靠的回復 SQL。",
  ...
  backendErrors: {
    ...
    transaction: {
      sessionExpired: "手動交易因 {timeoutSecs} 秒無操作而過期並回滾，PanDB 將自動開始新的交易。",
    },
    legacy: "後端回傳了錯誤。",
    unknown: "發生未預期的後端錯誤。",
  },
  ...
  ai: {
    ...
    closeWithRunsDescription: "結束 PanDB 會中斷 {count} 個正在執行或排隊的 AI 任務。你也可以取消結束並等待任務完成。",
    ...
    productionWriteBlocked: "AI 無法在生產資料庫執行資料或結構變更。請核對此 SQL 後在 PanDB 中手動執行。",
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "設定覆寫了由 PanDB 管理的 MCP 環境變數。請從供應商設定中移除 PanDB_MCP_* 變數。",
      ...
      piAgentEnvReserved: "設定覆寫了由 PanDB 管理的 Pi 或 MCP 環境變數。請移除 PanDB_PI_* 與 PanDB_MCP_* 變數。",
      ...
      openCodeEnvReserved: "設定覆寫了由 OpenCode 或 PanDB 管理的環境變數。請移除 OPENCODE 控制變數和 PanDB_MCP_* 變數。",
      openCodeNotAuthenticated: "OpenCode 尚未設定已驗證的模型供應商。請執行 `opencode auth login` 或在 OpenCode 中設定供應商後重試。",
      openCodeMcpStartupFailed: "OpenCode 無法啟動受限範圍的 PanDB MCP Server。請檢查 設定 > MCP 和下方診斷詳情。",
      ...
      cursorEnvReserved: "設定覆寫了由 Cursor 或 PanDB 管理的環境變數。請移除 CURSOR_CONFIG_DIR、CURSOR_DATA_DIR 和 PanDB_MCP_* 變數。",
      cursorNotAuthenticated: "Cursor CLI 尚未登入。請在終端執行 `agent login` 後重試。",
      cursorMcpStartupFailed: "Cursor 無法啟動受限範圍的 PanDB MCP Server。請檢查 設定 > MCP 和下方診斷詳情。",
      ...
      grokCliEnvReserved: "覆寫了 PanDB 管理的 MCP 環境變數。請從供應商設定中移除 PanDB_MCP_* 變數。",
      grokCliNotAuthenticated: "Grok CLI 尚未登入。請在終端機執行 `grok login` 後重試。",
      grokCliMcpStartupFailed: "Grok 已載入 MCP 設定，但無法啟動 PanDB MCP Server。請檢查 設定 > MCP 與下方診斷資訊。",
      grokCliCommandLineTooLong: "命令列過長，Windows 拒絕執行 Grok CLI。請更新 PanDB 並使用產生的 prompt 檔案重試。",
      ...
      codeBuddyEnvReserved: "覆寫了 PanDB 管理的 MCP 環境變數。請從供應商設定中移除 PanDB_MCP_* 變數。",
      codeBuddyNotAuthenticated: "CodeBuddy Code 尚未登入。請在終端機開啟 `codebuddy`，完成登入後重試。",
      codeBuddyTimeout: "CodeBuddy Code CLI 未在操作逾時前回應。",
      codeBuddyMcpConfigInvalid: "CodeBuddy Code 無法讀取暫存 PanDB MCP 設定。請檢查下方診斷資訊。",
      codeBuddyMcpStartupFailed: "CodeBuddy Code 已載入 MCP 設定，但無法啟動 PanDB MCP Server。請檢查 設定 > MCP 與下方診斷資訊。",
      ...
      qoderEnvReserved: "設定覆蓋了由 PanDB 管理的 MCP 環境變數。請從供應商設定中移除 PanDB_MCP_* 變數。",
      ...
      qoderMcpConfigInvalid: "Qoder CLI 無法讀取暫時的 PanDB MCP 設定。請檢查 Qoder CLI 版本和下方診斷詳情。",
      qoderMcpStartupFailed: "Qoder CLI 已載入 MCP 設定，但無法啟動 PanDB MCP Server。請檢查 設定 > MCP 和下方診斷詳情。",
  ...
  settings: {
    ...
    syncWebDavWebDescription: "Web 模式由 PanDB 伺服器端存取 WebDAV，不受瀏覽器 CORS 限制；儲存的密碼會保留在目前 PanDB 執行個體中。",
    ...
    syncRememberWebDavPasswordDescription: "密碼會加密儲存在目前 PanDB 執行個體中，不會同步到 WebDAV，也不會取代同步密碼。",
    ...
    syncSecretsPassphraseDescription: "此密碼只用於加密和還原敏感資料，並會加密儲存在目前 PanDB 執行個體中；在其他執行個體還原時需要再次輸入。",
    syncTest: "測試",
    syncUpload: "上傳",
    syncDownload: "下載",
    ...
    appCloseUnsavedTabsModeDescription: "選擇結束 PanDB 時如何處理有未儲存內容的 SQL 分頁。",
    appCloseUnsavedTabsModePrompt: "結束前詢問是否儲存",
    appCloseUnsavedTabsModeKeepDrafts: "直接結束並保留草稿",
    appCloseUnsavedTabsModeHint: "草稿會以未儲存分頁的形式保留，下次啟動 PanDB 時還原；需要「啟動時還原分頁」不是「不保留分頁」。",
    appCloseDraftPersistFailed: "PanDB 無法儲存開啟的分頁草稿，因此已取消退出。請檢查磁碟空間和檔案權限後重試。詳細資料：{message}",
  ...
  driverStore: {
    ...
    offlineExportSecurityHint: "僅包含 PanDB 託管的驅動程式產物及所需 JRE；本機路徑、設定和外部驅動程式不會被匯出。",
    ...
    offlineExportUnavailable_unmanagedInstall: "此產物未被記錄為 PanDB 託管安裝。",
    ...
    offlineExportUnavailable_missingManagedJre: "尚未安裝所需的 PanDB 託管 JRE。",
    offlineExportUnavailable_invalidManagedJre: "所需的 PanDB 託管 JRE 無效或不安全。",
    refresh: "重新整理",
    ...
    update: "更新",
    localInstall: "本機安裝",
    localInstallHint: "適用於完全內網環境：在另一台可連網的電腦上，從 PanDB 的 GitHub Releases 頁面下載官方 dbx-jdbc-plugin-*.zip 安裝包，傳輸到本機後在此選取該檔案——本機不需要連網即可完成安裝。",
    runtimeDrivers: "執行環境",
    storageTab: "儲存",
  ...
  nacos: {
    ...
    namespaceId: "命名空間 ID",
    namespaceIdPlaceholder: "留空則由 PanDB 自動產生 UUID",
    ...
    nacosDetectAccessibleNamespacesHint: "PanDB 會驗證「{name}」中每個命名空間的設定與服務讀取權限後再供選擇。",
    ...
    nacosV3ConsoleManagedNamespacesHint: "目前使用 Nacos 3 Console API（通常為 8080）。可填寫命名空間名稱或 ID；PanDB 能讀取目錄時會自動將名稱轉換為實際 ID。",
    rnacosNamespaceAccessScopeHint: "設定 r-nacos Console 使用者資料後，PanDB 會依目前使用者的命名空間權限決定存取範圍。",
    nacosAuth: "驗證",
