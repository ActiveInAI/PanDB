...
export default withEnglishFallback({
  ...
  updates: {
    ...
    portableAutomaticUpdate: "PanDB は署名済みのポータブル ZIP をダウンロードし、終了後に PanDB.exe のみを置き換えて自動的に再起動します。portable.dbx とデータは保持されます。",
  ...
  sqlLibrary: {
    title: "SQLライブラリ",
    storageHelp: "SQLライブラリの内容はPanDBのアプリデータに保存され、ディスク上の独立した.sqlファイルではありません。\nファイルシステムで使用するには、SQLライブラリをエクスポートしてください。",
    unassociated: "未関連付け",
  ...
  connection: {
    ...
    rememberPasswordSaveFailed: "接続しましたが、PanDB はパスワードを保存できませんでした: {message}",
  ...
  grid: {
    ...
    conditionalBulkEditConfirmMessage: "実行時に条件へ一致するすべての行（未読込の行を含む）を直ちに更新します。PanDB は信頼できるロールバック SQL を生成できません。",
  ...
  ai: {
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "PanDBが管理するMCP環境変数が上書きされています。プロバイダー設定からPanDB_MCP_*変数を削除してください。",
      ...
      piAgentEnvReserved: "PanDBが管理するPiまたはMCP環境変数が上書きされています。PanDB_PI_*とPanDB_MCP_*を削除してください。",
      ...
      openCodeEnvReserved: "OpenCodeまたはPanDBが管理する環境変数が上書きされています。設定からOPENCODE制御変数とPanDB_MCP_*を削除してください。",
      openCodeNotAuthenticated: "OpenCodeに認証済みのモデルプロバイダーがありません。`opencode auth login`を実行するかOpenCodeでプロバイダーを設定して、再試行してください。",
      openCodeMcpStartupFailed: "OpenCodeはスコープ付きPanDB MCPサーバーを起動できませんでした。設定 > MCPと以下の診断詳細を確認してください。",
      ...
      cursorEnvReserved: "CursorまたはPanDBが管理する環境変数が上書きされています。CURSOR_CONFIG_DIR、CURSOR_DATA_DIR、PanDB_MCP_*を設定から削除してください。",
      cursorNotAuthenticated: "Cursor CLIにログインしていません。ターミナルで`agent login`を実行してから再試行してください。",
      cursorMcpStartupFailed: "Cursorはスコープ付きPanDB MCPサーバーを起動できませんでした。設定 > MCPと以下の診断詳細を確認してください。",
      ...
      grokCliEnvReserved: "PanDBが管理するMCP環境変数が上書きされています。プロバイダー設定からPanDB_MCP_*変数を削除してください。",
      grokCliNotAuthenticated: "Grok CLIにログインしていません。ターミナルで`grok login`を実行してから再試行してください。",
      grokCliMcpStartupFailed: "GrokはMCP設定を読み込みましたが、PanDB MCP Serverを起動できません。設定 > MCPと以下の診断詳細を確認してください。",
      grokCliCommandLineTooLong: "Windowsが長すぎるGrok CLIコマンドを拒否しました。PanDBを更新し、生成されたプロンプトファイルで再試行してください。",
      ...
      codeBuddyEnvReserved: "PanDBが管理するMCP環境変数が上書きされています。プロバイダー設定からPanDB_MCP_*変数を削除してください。",
      ...
      codeBuddyMcpConfigInvalid: "CodeBuddy Codeは一時的なPanDB MCP設定を読み込めませんでした。以下の診断情報を確認してください。",
      codeBuddyMcpStartupFailed: "CodeBuddy CodeはMCP設定を読み込みましたが、PanDB MCP Serverを起動できません。設定 > MCPと以下の診断情報を確認してください。",
      ...
      qoderEnvReserved: "PanDBが管理するMCP環境変数が上書きされました。プロバイダー設定からPanDB_MCP_*変数を削除してください。",
      ...
      qoderMcpConfigInvalid: "Qoder CLIが一時的なPanDB MCP設定を読み取れませんでした。Qoder CLIのバージョンと以下の診断詳細を確認してください。",
      qoderMcpStartupFailed: "Qoder CLIはMCP設定を読み込みましたが、PanDB MCP Serverを起動できません。設定 > MCPと以下の診断詳細を確認してください。",
    ...
    closeWithRunsDescription: "PanDBを終了すると、実行中またはキューに入っている{count}件のAIタスクが中断されます。このダイアログを閉じて完了を待つこともできます。",
    ...
    productionWriteBlocked: "AI は本番データベースでデータまたはスキーマの変更を実行できません。この SQL を確認し、PanDB で手動実行してください。",
  ...
  settings: {
    ...
    syncWebDavWebDescription: "WebモードではPanDBサーバーがWebDAVへ接続するため、ブラウザーのCORS制限を受けません。保存したパスワードは現在のPanDBインスタンス内に保持されます。",
    ...
    syncRememberWebDavPasswordDescription: "パスワードは暗号化され、現在のPanDBインスタンスにのみ保存されます。WebDAVには同期されず、同期パスワードを置き換えるものでもありません。",
    ...
    syncSecretsPassphraseDescription: "このパスワードは機密データの暗号化と復元にのみ使用され、現在のPanDBインスタンスに暗号化して保存されます。他のインスタンスでは再入力してください。",
    syncTest: "テスト",
    syncUpload: "アップロード",
    syncDownload: "ダウンロード",
    ...
    appCloseUnsavedTabsModeDescription: "未保存の編集があるSQLタブが開いたままPanDBを終了するときの動作を選択します。",
    appCloseUnsavedTabsModePrompt: "保存するかどうか確認する",
    appCloseUnsavedTabsModeKeepDrafts: "確認せず終了して下書きを保持",
    appCloseUnsavedTabsModeHint: "下書きは未保存のタブとして保持され、次回PanDBを起動したときに復元されます。「起動時にタブを復元」が「タブを保持しない」になっていない必要があります。",
    appCloseDraftPersistFailed: "開いているタブの下書きを保存できなかったため、PanDB の終了を中止しました。ディスク容量とファイル権限を確認してから、もう一度終了してください。詳細: {message}",
  ...
  driverStore: {
    ...
    offlineExportDescription: "含めるPanDB管理のAgentドライバーを選択します。パッケージは現在のプラットフォームでのみ使用できます。",
    ...
    offlineExportNoDrivers: "エクスポート可能なPanDB管理のAgentドライバーがインストールされていません。",
    ...
    offlineExportSecurityHint: "PanDB管理の成果物と必要なJREのみを含めます。ローカルパス、設定、外部ドライバーは除外されます。",
    ...
    offlineExportUnavailable_unmanagedInstall: "この成果物はPanDB管理のインストールとして記録されていません。",
    ...
    offlineExportUnavailable_missingManagedJre: "必要なPanDB管理JREがインストールされていません。",
    offlineExportUnavailable_invalidManagedJre: "必要なPanDB管理JREが無効または安全ではありません。",
    refresh: "更新",
    ...
    localInstallHint: "完全に隔離されたネットワーク環境の場合：インターネットに接続できる別のコンピューターで PanDB の GitHub Releases ページから公式の dbx-jdbc-plugin-*.zip をダウンロードし、このマシンに転送して選択してください。このマシンはネットワークに接続する必要はありません。",
  ...
  nacos: {
    ...
    namespaceId: "名前空間ID",
    namespaceIdPlaceholder: "空欄の場合はPanDBがUUIDを生成",
    namespaceName: "名前空間名",
    ...
    nacosDetectAccessibleNamespacesHint: "PanDB は {name} の名前空間を提示する前に、設定とサービスの読み取りアクセスを検証します。",
    ...
    nacosV3ConsoleManagedNamespacesHint: "この接続は Nacos 3 Console API（通常はポート 8080）を使用します。名前空間名または ID を入力できます。ディレクトリを読み取れる場合、PanDB は名前を実際の ID に変換します。",
